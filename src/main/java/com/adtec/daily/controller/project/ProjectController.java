...
public class ProjectController {
    ...
    public Msg saveProject(@Valid Project project, BindingResult result) {
        if (result.hasErrors()) {
            ...
            ProjectExample.Criteria cri2 = projectExample.createCriteria();
            cri2.andProjectUserIdEqualTo(project.getProjectUserId());
            List<Project> projectLists2 = projectService.getProjectByUser(project.getProjectUserId());
            if(projectLists2.size() > 0) {
                return Msg.dateREExist();
            }
